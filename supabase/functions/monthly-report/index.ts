...
serve(async (req) => {
  ...
  try {
    ...
    if (!month || !/^\d{4}-\d{2}$/.test(month)) {
      return new Response(JSON.stringify({ error: "Mois invalide (attendu YYYY-MM)" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    const [docsRes, versionsRes, ratesRes, tasksRes, meetingsRes] = await Promise.all([
      supabase.from("documents").select("id,name,category,mime_type,size_bytes,created_at,updated_at,tags").gte("created_at", start).lt("created_at", end),
      supabase.from("document_versions").select("id,document_id,version_number,created_at,size_bytes,comment").gte("created_at", start).lt("created_at", end),
      supabase.from("exchange_rates").select("rate_date,usd_to_fc,eur_to_usd,chf_to_usd,updated_at").gte("rate_date", start).lt("rate_date", end).order("rate_date", { ascending: true }),
      supabase.from("tasks").select("id,title,status,priority,due_date,created_at").gte("created_at", start).lt("created_at", end),
      supabase.from("meetings").select("id,title,meeting_date,participants,summary").gte("meeting_date", start).lt("meeting_date", end),
    ]);
    ...
    const rateStats = (key: "usd_to_fc" | "eur_to_usd" | "chf_to_usd") => {
      const values = rates.map((r: any) => r[key]).filter((v: any) => v !== null && v !== undefined) as number[];
    ...
    const stats = {
      documents: {
        ...
        byCategory: docs.reduce((acc: Record<string, number>, d: any) => { const k = d.category ?? "Sans catégorie"; acc[k] = (acc[k] ?? 0) + 1; return acc; }, {}),
      },
      ...
      tasks: {
        count: tasks.length,
        byStatus: tasks.reduce((acc: Record<string, number>, t: any) => { acc[t.status] = (acc[t.status] ?? 0) + 1; return acc; }, {}),
      },
...
Réponds UNIQUEMENT avec le JSON valide, sans markdown.`;
    ...
    if (!aiRes.ok) {
      if (aiRes.status === 429) return new Response(JSON.stringify({ error: "Trop de requêtes IA, réessayez plus tard." }), { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    catch { report = { executive_summary: raw, key_points: [], rate_analysis: "", activity_analysis: "", recommendations: [] }; }
