...
import { TrendingUp, FileText, Users2, ListTodo, CalendarDays, HardDrive, Clock } from "lucide-react";
...
type Rate = { id?: string; rate_date: string; usd_to_fc: number | null; eur_to_usd: number | null; chf_to_usd: number | null; updated_at?: string };

function RateField({ label, value, onSave }: { label: string; value: number | null; onSave: (n: number) => Promise<void> }) {
  const [text, setText] = useState(value !== null && value !== undefined ? value.toFixed(6) : "");
  const [saving, setSaving] = useState(false);

  useEffect(() => { setText(value !== null && value !== undefined ? value.toFixed(6) : ""); }, [value]);

  const commit = async () => {
    const n = parseFloat(text.replace(",", "."));
    if (Number.isNaN(n) || n < 0) { toast.error("Valeur invalide"); return; }
    setSaving(true);
    try { await onSave(n); setText(n.toFixed(6)); }
  ...
  return (
    <div className="rounded-lg border bg-card p-4 transition-colors hover:border-primary/40">
      ...
      <input
        ...
        disabled={saving}
      />
      <div className="mt-1 text-[11px] text-muted-foreground">{saving ? "Enregistrement…" : "Cliquez pour modifier · 6 décimales"}</div>
    </div>
  );
}
...
function Dashboard() {
  ...
  const load = async () => {
    const { data: r } = await supabase.from("exchange_rates").select("*").order("rate_date", { ascending: false }).limit(8);
    ...
    if (user) {
      const [{ count: dc }, { count: mc }, { count: tc }, { count: ec }] = await Promise.all([
        ...
        supabase.from("calendar_events").select("*", { count: "exact", head: true }).eq("user_id", user.id),
      ]);
      ...
      <section className="rounded-xl border bg-card p-5 shadow-sm">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="text-xs text-muted-foreground">Sauvegarde automatique · historique conservé par jour</div>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <RateField label="USD → FC" value={rate?.usd_to_fc ?? null} onSave={(n) => saveField("usd_to_fc", n)} />
          <RateField label="EUR → USD" value={rate?.eur_to_usd ?? null} onSave={(n) => saveField("eur_to_usd", n)} />
          <RateField label="Franc Suisse → USD" value={rate?.chf_to_usd ?? null} onSave={(n) => saveField("chf_to_usd", n)} />
        </div>
              ...
              <tbody>
                {history.map((h) => (
                  <tr key={h.rate_date} className="border-t">
                    <td className="px-3 py-2">{new Date(h.rate_date).toLocaleDateString("fr-FR")}</td>
                    <td className="px-3 py-2 text-right font-mono">{h.usd_to_fc?.toFixed(6) ?? "—"}</td>
                    <td className="px-3 py-2 text-right font-mono">{h.eur_to_usd?.toFixed(6) ?? "—"}</td>
                    <td className="px-3 py-2 text-right font-mono">{h.chf_to_usd?.toFixed(6) ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-xl border bg-card p-5 lg:col-span-2">
          ...
          <div className="space-y-3 text-sm text-muted-foreground">
            <div className="flex items-center justify-between border-b pb-2"><span>Connexion à l'espace</span><span className="text-xs">{now.toLocaleTimeString("fr-FR")}</span></div>
            <div className="flex items-center justify-between border-b pb-2"><span>Dernière session active</span><span className="text-xs">aujourd'hui</span></div>
            <div className="flex items-center justify-between"><span>Statut système</span><span className="text-xs text-success">Opérationnel</span></div>
          </div>
        </div>
        <div className="rounded-xl border bg-card p-5">
          <div className="mb-3 flex items-center gap-2 text-sm font-semibold"><HardDrive className="h-4 w-4 text-primary" /> Stockage</div>
          <div className="text-2xl font-semibold">{stats.docs} fichiers</div>
          <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-muted">
            <div className="h-full bg-primary" style={{ width: `${Math.min(stats.docs * 2, 100)}%` }} />
          </div>
