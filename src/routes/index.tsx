...
export const Route = createFileRoute("/")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Documents, réunions, tâches, calendrier et IA dans une plateforme unifiée." },
  ]}),
  component: Home,
});
    ...
    <div className="min-h-screen bg-background">
      <header className="flex items-center justify-between px-6 py-5 lg:px-12">
        <div className="flex items-center gap-2">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary text-primary-foreground"><Cloud className="h-5 w-5" /></div>
      ...
      <main className="mx-auto max-w-5xl px-6 pb-24 pt-12 lg:pt-20">
        <div className="text-center">
          ...
          <h1 className="mt-6 text-4xl font-semibold tracking-tight sm:text-5xl lg:text-6xl">
            Votre espace de travail<br /><span className="text-primary">unifié et sécurisé.</span>
          </h1>
          <p className="mx-auto mt-5 max-w-xl text-base text-muted-foreground">
            Documents, réunions, calendrier, tâches, OCR et assistant IA — tout dans une interface professionnelle, en français, sur tous vos appareils.
          </p>
          <div className="mt-8 flex justify-center gap-3">
            <Link to="/login"><Button size="lg">Démarrer<ArrowRight className="ml-2 h-4 w-4" /></Button></Link>
          </div>
        </div>
        ...
        <div className="mt-20 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {features.map((f) => (
            <div key={f.title} className="rounded-xl border bg-card p-5">
              <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-md bg-primary/10 text-primary"><f.icon className="h-4 w-4" /></div>
