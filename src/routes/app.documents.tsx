...
function DocsPage() {
  ...
  const createFolder = async () => {
    ...
    const { error } = await supabase
      .from("folders")
      .insert({ user_id: user.id, name, parent_id: folderId, kind: "document" });
  ...
  const moveToNewFolder = async (d: Doc) => {
    ...
    const { data: nf, error } = await supabase
      .from("folders")
      .insert({ user_id: user.id, name, kind: "document" })
      .select()
      .single();
      ...
      <div className="grid gap-4 lg:grid-cols-[16rem_1fr]">
        <aside className="space-y-1 rounded-xl border bg-card p-3">
          <button
            ...
            className={`flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm ${view === "mine" ? "bg-accent" : "hover:bg-accent/60"}`}
          >
            <Folder className="h-4 w-4" /> Mes fichiers
          </button>
          <button
            onClick={() => setView("shared")}
            className={`flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm ${view === "shared" ? "bg-accent" : "hover:bg-accent/60"}`}
          >
          ...
          <button
            ...
            className={`flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm ${!folderId ? "bg-accent" : "hover:bg-accent/60"}`}
          >
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-right">
                        {isOwner && (
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button size="icon" variant="ghost" title="Déplacer vers un dossier">
                            ...
                            <DropdownMenuContent align="end" className="w-56">
                              ...
                              <DropdownMenuSeparator />
                              <DropdownMenuItem
                                onClick={() => moveDoc(d, null)}
                                disabled={!d.folder_id}
                              >
                              ...
                              {folders.map((f) => (
                                <DropdownMenuItem
                                  key={f.id}
                                  onClick={() => moveDoc(d, f.id)}
                                  disabled={d.folder_id === f.id}
                                >
                        ...
                        {isOwner && (
                          <Button size="icon" variant="ghost" onClick={() => setShareDocId(d.id)}>
        ...
        <DialogContent>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              L'extension .docx sera ajoutée automatiquement.
            </p>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setNoteOpen(false)} disabled={creatingNote}>
              Annuler
            </Button>
            <Button onClick={submitNote} disabled={creatingNote}>
              {creatingNote ? "Création…" : "OK"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
