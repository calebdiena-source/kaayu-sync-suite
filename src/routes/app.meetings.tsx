...
function MeetingsPage() {
  ...
  const load = async () => {
    if (!user) return;
    const { data } = await supabase.from("meetings").select("*").eq("user_id", user.id).order("meeting_date", { ascending: false });
    setList(data ?? []);
  };
  ...
  const create = async () => {
    ...
    const { error } = await supabase.from("meetings").insert({
      user_id: user.id, title, meeting_date: date,
      participants: participants.split(",").map((s) => s.trim()).filter(Boolean),
      notes,
    });
    if (error) toast.error(error.message);
    else { toast.success("Réunion créée"); setShowNew(false); setTitle(""); setNotes(""); setParticipants(""); load(); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setShowNew(!showNew)}><Plus className="mr-2 h-4 w-4" />Nouvelle réunion</Button>
      </div>

      {showNew && (
        <div className="space-y-3 rounded-xl border bg-card p-5">
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Titre" className="w-full rounded-md border bg-background px-3 py-2 text-sm" />
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <input value={participants} onChange={(e) => setParticipants(e.target.value)} placeholder="Participants (séparés par des virgules)" className="rounded-md border bg-background px-3 py-2 text-sm" />
          </div>
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Notes…" rows={4} className="w-full rounded-md border bg-background px-3 py-2 text-sm" />
          <div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setShowNew(false)}>Annuler</Button><Button onClick={create}>Créer</Button></div>
        </div>
      )}
      ...
      {Object.entries(grouped).map(([day, items]) => (
        <div key={day}>
          <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            <CalIcon className="h-3.5 w-3.5" /> {new Date(day).toLocaleDateString("fr-FR", { dateStyle: "full" })}
          </div>
              ...
              <div key={m.id} className="rounded-xl border bg-card p-5">
                ...
                {m.summary && (
                  <div className="mt-3 rounded-md border-l-2 border-primary bg-primary/5 p-3 text-sm">
                    <div className="mb-1 text-xs font-semibold uppercase tracking-wider text-primary">Résumé IA</div>
      ...
      {list.length === 0 && <div className="rounded-xl border bg-card p-12 text-center text-sm text-muted-foreground">Aucune réunion. Créez-en une pour commencer.</div>}
    </div>
  );
}
