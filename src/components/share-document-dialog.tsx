...
export function ShareDocumentDialog({
  ...
  const filtered = profiles.filter(
    (p) =>
      ...
      !shares.some((s) => s.shared_with_user_id === p.id),
  );
      ...
      <DialogContent>
        ...
        <div className="space-y-4">
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((p) => (
                  <button
                    ...
                    className={`flex w-full items-center justify-between px-3 py-2 text-sm hover:bg-accent ${selected === p.id ? "bg-accent" : ""}`}
                  >
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={add} disabled={!selected} className="ml-auto">
              ...
              {shares.map((s) => (
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground">
                    {s.permission === "write" ? "Écriture" : "Lecture"}
                  </span>
